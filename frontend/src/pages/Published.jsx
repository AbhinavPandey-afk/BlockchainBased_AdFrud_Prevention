...
// Gateway resolver
const lighthouseGateway = (path) => {
  const gw = process.env.REACT_APP_LIGHTHOUSE_GATEWAY || 'https://gateway.lighthouse.storage';
  if (!path) return null;
  if (path.startsWith('ipfs://')) {
    const after = path.replace('ipfs://', '');
    return `${gw}/ipfs/${after}`;
  }
  if (/^[a-zA-Z0-9]+$/.test(path) && path.length > 40) {
    return `${gw}/ipfs/${path}`;
  }
  return path;
};

// Extract { cid, path } from ipfs://<cid>/<path?>
const parseIpfsParts = (ipfsUri) => {
  if (!ipfsUri?.startsWith('ipfs://')) return { cid: null, path: null };
  const after = ipfsUri.replace('ipfs://', '');
  const slash = after.indexOf('/');
  if (slash === -1) return { cid: after, path: "" };
  return { cid: after.slice(0, slash), path: after.slice(slash + 1) };
};

// Given metadata.image and metaUri (ipfs://<dirCID>/metadata.json), produce a displayable gateway URL
const resolveImageUrl = (metaImage, metaUri) => {
  if (!metaImage) return null;
  if (metaImage.startsWith('ipfs://')) {
    return lighthouseGateway(metaImage);
  }
  // Relative path inside the same directory (e.g., "./image.png" or "image.png")
  const { cid } = parseIpfsParts(metaUri);
  if (!cid) return null;
  const cleaned = metaImage.startsWith('./') ? metaImage.slice(2) : metaImage;
  return lighthouseGateway(`ipfs://${cid}/${cleaned}`);
};

// Fetch campaign metadata from meta field
const fetchCampaignMetadata = async (metaString, campaignId, apiKey) => {
  try {
    if (metaString && metaString.startsWith('ipfs://')) {
      // Fetch; if it's a directory pointer, fallback to metadata.json
      let url = lighthouseGateway(metaString);
      let resp = await fetch(url, { headers: apiKey ? { 'Authorization': `Bearer ${apiKey}` } : {} });
      if (resp.ok) {
        ...
        try {
          ...
          const { cid } = parseIpfsParts(metaString);
          if (cid) {
            const metaUrl = lighthouseGateway(`ipfs://${cid}/metadata.json`);
            const r2 = await fetch(metaUrl, { headers: apiKey ? { 'Authorization': `Bearer ${apiKey}` } : {} });
    ...
    console.warn("Metadata fetch failed for campaign", campaignId, e);
    return { description: metaString || "", image: null };
  }
};
...
const Published = () => {
  ...
  const contractAddress = process.env.REACT_APP_CONTRACT_ADDRESS;
  const recaptchaSiteKey = process.env.REACT_APP_RECAPTCHA_SITE_KEY;
  const lighthouseApiKey = process.env.REACT_APP_LIGHTHOUSE_API_KEY || "";
  ...
  // Fetch campaigns + resolve Lighthouse metadata/image
  const fetchCampaigns = async () => {
    if (!contract) return;
    try {
      setLoading(true);
      ...
      for (let id of campaignIds) {
        try {
          ...
          if (c[2].gt(0) && !c[3]) {
            const metaResolved = await fetchCampaignMetadata(c[4], id.toString(), lighthouseApiKey);
            let imageUrl = null;
            if (metaResolved?.image) {
              imageUrl = resolveImageUrl(metaResolved.image, c[4]);
            }
            campaignList.push({
              id: id.toString(),
              advertiser: c[0],
              cpcWei: c[1].toString(),
              budgetWei: c[2].toString(),
              paused: c[3],
              meta: c[4],
              _metaResolved: {
                description: metaResolved?.description || "",
                image: imageUrl,
              },
            });
          }
        } catch (error) {
          console.error(`Failed to fetch/resolve campaign ${id}:`, error);
        }
      }
      setCampaigns(campaignList);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const setupEventListeners = async () => {
      try {
        const campaignCreatedFilter = contract.filters.CampaignCreated();
        contract.on(campaignCreatedFilter, async (campaignId, advertiser, cpcWei, initialBudget, meta) => {
          if (initialBudget.gt(0)) {
            const resolved = await fetchCampaignMetadata(meta, campaignId.toString(), lighthouseApiKey);
            const imageUrl = resolved?.image ? resolveImageUrl(resolved.image, meta) : null;
            const newCampaign = {
              id: campaignId.toString(),
              advertiser,
              cpcWei: cpcWei.toString(),
              ...
              paused: false,
              meta,
              _metaResolved: { description: resolved?.description || "", image: imageUrl },
            };
            setCampaigns(prev => (prev.find(c => c.id === newCampaign.id) ? prev : [...prev, newCampaign]));
          }
        });

        contract.on("CampaignFunded", (campaignId, amount, newBudget) => {
          setCampaigns(prev => prev.map(c =>
            c.id === campaignId.toString() ? { ...c, budgetWei: newBudget.toString() } : c
          ));
        });

        contract.on("CampaignPaused", (campaignId, paused) => {
          setCampaigns(prev => prev.map(c =>
            c.id === campaignId.toString() ? { ...c, paused } : c
          ));
        });
      } catch (error) {
        console.error("Failed to set up event listeners:", error);
  ...
  }, [contract, lighthouseApiKey]);

  useEffect(() => { if (contract) fetchCampaigns(); }, [contract]);
  ...
  // CAPTCHA handlers
  const onCaptchaChange = (value) => { if (value) { setCaptchaVerified(true); setCaptchaLoading(false); } else { setCaptchaVerified(false); } };
  const onCaptchaExpired = () => { setCaptchaVerified(false); setMsg("❌ CAPTCHA expired. Please verify again."); };
  const onCaptchaError = () => { setCaptchaVerified(false); setMsg("❌ CAPTCHA error. Please try again."); };

  const handleAdClick = async (campaign) => {
    if (!isConnected || !account) { setMsg("❌ Please connect your wallet first to generate clicks"); return; }
    setSelectedCampaign(campaign);
    setShowCaptchaModal(true);
    setCaptchaVerified(false);
    setCaptchaLoading(false);
  ...
  const processCaptchaAndClick = async () => {
    if (!captchaVerified || !selectedCampaign) { setMsg("❌ Please complete the CAPTCHA verification"); return; }
    try {
      setCaptchaLoading(true);
      const timestamp = Math.floor(Date.now() / 1000);
      const clickHash = await generateClickHash(selectedCampaign.id, account, timestamp);
      const newHash = { clickHash, campaignId: selectedCampaign.id, publisherAddress: account, timestamp, cpc: ethers.utils.formatEther(selectedCampaign.cpcWei) };
      setGeneratedHashes(prev => [newHash, ...prev.slice(0, 9)]);
      setMsg(`✅ Click registered successfully! Hash: ${clickHash.substring(0, 10)}...`);
      const existingHashes = JSON.parse(localStorage.getItem('availableHashes') || '[]');
      existingHashes.unshift(newHash); existingHashes.splice(50);
      localStorage.setItem('availableHashes', JSON.stringify(existingHashes));
      setShowCaptchaModal(false); setSelectedCampaign(null); setCaptchaVerified(false);
  ...
  if (loading) {
    return (
      <Container className="py-4 text-center">
        <Spinner animation="border" role="status"><span className="visually-hidden">Loading campaigns...</span></Spinner>
        <p className="mt-2">Loading campaigns from blockchain...</p>
      </Container>
    );
  }
  ...
  return (
    <Container className="py-4">
      ...
      <div className="mb-3 d-flex justify-content-between align-items-center">
        <h3>Active Ad Campaigns ({campaigns.length})</h3>
        <Button variant="outline-primary" onClick={fetchCampaigns} size="sm">🔄 Refresh</Button>
      </div>

      <Row>
        <Col md={8}>
          {campaigns.length === 0 ? (
            <Alert variant="info">No active campaigns found. Create a campaign first.</Alert>
          ) : (
            <Row>
              {campaigns.map((campaign) => {
                const metadata = campaign._metaResolved || { description: "", image: null };
                return (
                  <Col md={6} lg={4} key={campaign.id} className="mb-4">
                    <Card className="h-100 shadow-sm border-0">
                      <div className="position-relative">
                        <Badge bg="primary" className="position-absolute top-0 start-0 m-2" style={{ zIndex: 10, fontSize: '0.8rem' }}>
                          Campaign #{campaign.id}
                        </Badge>
                        <div style={{ height: '200px', overflow: 'hidden' }}>
                          {metadata.image ? (
                            <Card.Img
                              ...
                              onError={(e) => {
                                console.error("Image failed to load for campaign", campaign.id, metadata.image);
                                e.target.style.display = 'none';
                                e.target.nextSibling.style.display = 'flex';
                              }}
                            />
                      ...
                      <Card.Body className="d-flex flex-column">
                        <div className="mb-2">
                          <h5 className="card-title mb-1 text-primary">Campaign #{campaign.id}</h5>
                          {metadata.description && (
                            <Card.Text className="text-muted small">
                              {metadata.description.substring(0, 80)}
                              {metadata.description.length > 80 ? '...' : ''}
        ...
        <Col md={4}>
          <Card className="shadow-sm border-0">
            <Card.Header className="bg-success text-white">
              <h4 className="mb-0"><span className="me-2">🎯</span>Recent Clicks</h4>
            </Card.Header>
            <Card.Body className="p-0">
              <div style={{ maxHeight: '400px', overflowY: 'auto' }}>
                {generatedHashes.length === 0 ? (
                  <div className="text-center py-4">
                    <span style={{ fontSize: '2rem', opacity: 0.5 }}>🎯</span>
                    <p className="text-muted small mt-2">
                      {!isConnected ? "Connect wallet to generate clicks" : "No clicks generated yet."}
                    </p>
                  </div>
                ) : (
                  generatedHashes.map((hash, index) => (
                    <div key={index} className="border-bottom p-3">
                      ...
                      <div className="small mb-2">
                        ...
                        <div><strong>CPC:</strong> {hash.cpc} ETH</div>
                        <div><strong>Publisher:</strong> {hash.publisherAddress.substring(0, 10)}...</div>
                        <div className="font-monospace"><strong>Hash:</strong> {hash.clickHash.substring(0, 20)}...</div>
                      </div>
                      <small className="text-muted">{new Date(hash.timestamp * 1000).toLocaleTimeString()}</small>
                    </div>
                  ))
                )}
              </div>
      ...
      <Modal show={showCaptchaModal} onHide={handleModalClose} centered>
        <Modal.Header closeButton>
          <Modal.Title><span className="me-2">🛡️</span>Security Verification Required</Modal.Title>
        </Modal.Header>
        <Modal.Body className="text-center">
          <div className="mb-4">
            <h5>Campaign #{selectedCampaign?.id}</h5>
            <p className="text-muted">Please complete the security verification to proceed with clicking this ad.</p>
            <div className="small text-success">
              <strong>Reward: {selectedCampaign ? ethers.utils.formatEther(selectedCampaign.cpcWei) : '0'} ETH</strong>
            </div>
          </div>
          ...
          {recaptchaSiteKey ? (
            ...
            <Alert variant="warning">reCAPTCHA site key not configured. Please add REACT_APP_RECAPTCHA_SITE_KEY to your environment variables.</Alert>
          )}

          {captchaVerified && <Alert variant="success">✅ Verification successful! You can now process the click.</Alert>}
        </Modal.Body>
        <Modal.Footer>
          <Button variant="secondary" onClick={handleModalClose}>Cancel</Button>
          <Button variant="success" onClick={processCaptchaAndClick} disabled={!captchaVerified || captchaLoading}>
            {captchaLoading ? (<><Spinner as="span" animation="border" size="sm" role="status" className="me-2" />Processing...</>) : (<><span className="me-2">🎯</span>Generate Click Hash</>)}
          </Button>
        </Modal.Footer>
      </Modal>
    </Container>
